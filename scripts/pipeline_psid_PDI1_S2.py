...
Usage:
    python scripts/pipeline_psid_PDI1_S2.py 2>&1 | tee psid_PDI1_S2.log
"""
...
# ── Logging ───────────────────────────────────────────────────────────────────
LOG_PATH = PROJECT_ROOT / "psid_PDI1_S2.log"
logging.basicConfig(
    level=logging.INFO,
...
GS_BASE = "psid_gs_PDI1_S2_200Hz_narrow_band"
...
def variant_name(nx: int, n1: int, dbs: str, vanilla: bool = False, i: int | None = None) -> str:
    ...
    return f"psid_behavioral_PDI1_2_nx_{nx}_n{n1}_i{ii}_{tag}dbs_{dbs}_200Hz_narrow_band"
...
def phase6_thesis(nx: int, n1: int, timestamps: dict[str, str],
    ...
    text = update_triplet(text, "_TRIPLET_PDI1_S2")
    text = update_triplet(text, "_NEURAL_BAND_TRIPLET_PDI1_S2")

    # Classification F1 pickle paths
    for group, label in [("xp", "Xp"), ("xp_1", "Xp_1"),
                         ("xp_2", "Xp_2"), ("xp_with_dbs", "Xp_with_dbs")]:
        pat = rf'(ClassificationF1PickleRef\("PDI1",\s*"S2",\s*"{group}",\s*")[^"]*(")'
...
def main():
    ...
    log.info("=" * 60)
    log.info("Overnight PSID pipeline — PDI1 S2, 200 Hz narrow band")

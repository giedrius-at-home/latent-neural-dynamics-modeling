...
Usage:
    python scripts/pipeline_psid_PDI4_S3.py 2>&1 | tee psid_PDI4_S3.log
"""
...
# ── Logging ───────────────────────────────────────────────────────────────────
LOG_PATH = PROJECT_ROOT / "psid_PDI4_S3.log"
logging.basicConfig(
    level=logging.INFO,
...
GS_BASE = "psid_gs_PDI4_S3_200Hz_narrow_band"
...
def variant_name(nx: int, n1: int, dbs: str, vanilla: bool = False, i: int | None = None) -> str:
    ...
    return f"psid_behavioral_PDI4_3_nx_{nx}_n{n1}_i{ii}_{tag}dbs_{dbs}_200Hz_narrow_band"
...
def phase6_thesis(nx: int, n1: int, timestamps: dict[str, str],
    ...
    for group, label in [("xp", "Xp"), ("xp_1", "Xp_1"),
        ...
        pat = rf'(ClassificationF1PickleRef\("PDI4",\s*"S3",\s*"{group}",\s*")[^"]*(")'
...
def main():
    ...
    log.info("=" * 60)
    log.info("Overnight PSID pipeline — PDI4 S3, 200 Hz narrow band")

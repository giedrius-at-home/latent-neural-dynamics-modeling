...
Usage:
    python scripts/pipeline_psid_PDI1_S4.py 2>&1 | tee psid_PDI1_S4.log
"""
...
# ── Logging ───────────────────────────────────────────────────────────────────
LOG_PATH = PROJECT_ROOT / "psid_PDI1_S4.log"
logging.basicConfig(
    level=logging.INFO,
...
FULL_I = 50          # full-training PSID iterations (swap-backed on brain)
MAX_EIGENVALUE = 0.999
GS_WORKERS = 2       # parallel workers (swap-backed on brain)

NX_GRID = [2, 4, 8, 15, 25, 30]
N1_GRID = [2, 4, 6]

GS_BASE = "psid_gs_PDI1_S4_200Hz_narrow_band"
...
def variant_name(nx: int, n1: int, dbs: str, vanilla: bool = False, i: int | None = None) -> str:
    ...
    return f"psid_behavioral_PDI1_4_nx_{nx}_n{n1}_i{ii}_{tag}dbs_{dbs}_200Hz_narrow_band"
...
def phase6_thesis(nx: int, n1: int, timestamps: dict[str, str],
    ...
    text = update_triplet(text, "_TRIPLET_PDI1_S4")
    text = update_triplet(text, "_NEURAL_BAND_TRIPLET_PDI1_S4")

    # Classification F1 pickle paths
    for group, label in [("xp", "Xp"), ("xp_1", "Xp_1"),
                         ("xp_2", "Xp_2"), ("xp_with_dbs", "Xp_with_dbs")]:
        pat = rf'(ClassificationF1PickleRef\("PDI1",\s*"S4",\s*"{group}",\s*")[^"]*(")'
...
def main():
    ...
    log.info("=" * 60)
    log.info("Overnight PSID pipeline — PDI1 S4, 200 Hz narrow band")

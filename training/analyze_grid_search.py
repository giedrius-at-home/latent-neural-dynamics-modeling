...
def load_validation_metrics(val_dir: Path) -> Dict[str, float]:
    ...
    if pearson_behavioral_trials:
        ...
        if valid:
            ...
            best_r = np.max(valid)
            concat_r = np.mean(overall_behavioral) if overall_behavioral else fisher_mean
...
def create_summary_dataframe(runs: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    sort_col = None
    for col in ['xcorr_mean_behavioral', 'concat_r_behavioral', 'fisher_mean_behavioral', 'median_behavioral']:
...
def print_best_runs(df: pd.DataFrame, top_n: int = 10):
    ...
    param_cols = ['nx', 'n1', 'alpha_Q', 'alpha_R', 'backward_kalman', 
                  'rescale_states', 'max_eigenvalue']
    metric_cols = ['xcorr_mean_behavioral', 'xcorr_best_behavioral',
                   'fisher_mean_behavioral', 'median_behavioral',
                   'best_behavioral', 'n_trials_behavioral']
...
def create_html_report(df: pd.DataFrame, output_path: Path):
    ...
    if 'concat_r_behavioral' in df.columns and len(df) > 0 and df['concat_r_behavioral'].notna().any():
...
def main():
    ...
    parser.add_argument('--output', type=str, default=None,
                        help='Output CSV path for full results')
    ...
    # Save to CSV
    output_csv = Path(args.output) if args.output else results_dir / 'grid_search_summary.csv'

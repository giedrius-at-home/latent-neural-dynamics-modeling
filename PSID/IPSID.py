...
def computeObsFromAC(A, C, i):
    ...
    Oy = C
    for ii in range(i):
        Oy = np.concatenate((Oy, Oy[(ii - 1) * ny : ii * ny, :] @ A))
    Oy_Minus = Oy[:(-ny), :]
    return Oy, Oy_Minus
...
def fitCzDzViaKFRegression(
    s, Y, Z, U=None, time_first=True, Cz=None, fit_Cz_via_KF=False, missing_marker=None
):
    """
    ...
    if fit_Cz_via_KF:
        if U is not None:
            CzDz = projOrth(
                ZTF[isNotMissing, :].T,
                np.concatenate((xHat[isNotMissing, :].T, UTF[isNotMissing, :].T)),
            )[1]
            nx = xHat.shape[1]
            Cz = CzDz[:, :nx]
            Dz = CzDz[:, nx:]
        ...
        if U is not None:
            Dz = projOrth(
                ZTF[isNotMissing, :].T - Cz @ xHat[isNotMissing, :].T,
                UTF[isNotMissing, :].T,
            )[1]
        else:
            Dz = None
...
def IPSID(
    ...
    * Comments within the documentation that refer to Eq. (XX), Figures, and Notes are referencing the above paper.
    ...
    if U is not None:
        UPrepModel.fit(
            U, remove_mean=remove_mean_U, zscore=zscore_U, time_first=time_first
        )
    ...
    if (
        nu == 0 or n1 == 0
    ):  # Since the external input U and/or n1 is not provided, preprocessing step is disabled and X3 won't be learned.
    ...
    if n1 > 0 and nz > 0:
        if n1 > iZ * nz:
            raise (
                Exception(
                    "n1 (currently {}) must be at most iZ*nz={}*{}={}. Use a larger horizon iZ.".format(
                        n1, iZ, nz, iZ * nz
                    )
                )
            )
        if "ZHatObUfRes_U" not in WS or WS["ZHatObUfRes_U"] is None:
            ...
            if remove_nonYrelated_fromX1:
                ...
                YHatOb_pr = projOblique(
                    WS["Yf"], np.concatenate((WS["Up"], WS["Yp"])), WS["Uf"]
                )[0]
                YHatObRes_pr = removeProjOrth(YHatOb_pr, WS["Uf"])
                U0, S0, YHat_V0 = linalg.svd(
                    YHatObRes_pr, full_matrices=False, lapack_driver="gesvd"
                )
                ...
                Zf_pre, Qz = projOblique(
                    Zf, Xk_pre, np.concatenate((WS["Up"], WS["Uf"]))
                )  # Eq.(39)

                Oy0_Minus = Oy0[:-ny, :]
                YHatMinus_pre = projOrth(
                    Yf_Minus,
                    np.concatenate(
            ...
            ZHatOb = projOblique(Zf, np.concatenate((WS["Up"], WS["Yp"])), WS["Uf"])[0]
            WS["ZHatObUfRes"] = removeProjOrth(ZHatOb, WS["Uf"])

            # Orthogonal projection of Zf onto UpYpUf
            WS["ZHat"] = projOrth(Zf, np.concatenate((WS["Up"], WS["Yp"], WS["Uf"])))[0]

            # Orthogonal projection of Zf_Minus onto Up_plus, Yp_plus and Uf_Minus
            WS["ZHatMinus"] = projOrth(
                Zf_Minus,
                np.concatenate((WS["Up"], WS["Uii"], WS["Yp"], WS["Yii"], Uf_Minus)),
            )[0]

            # Take SVD of ZHatObUfRes
            WS["ZHatObUfRes_U"], WS["ZHatObUfRes_S"], ZHat_V = linalg.svd(
                WS["ZHatObUfRes"], full_matrices=False, lapack_driver="gesvd"
            )  # Eq.(23)
    ...
    if (
        n3 > 0
    ):  # In case asked to dedicate some model capacity (state dimension) to X3, then recompute dimension of X21
        n2 = max(
            0, nx - n1 - n3
        )  # Anything remaining from nx after allocating n1 and n3 becomes n2
        n3 = (
            nx - n1 - n2
        )  # The dimension of final model would be equal to final n1+n2+n3 based on their adjusted values (which is equal to the input nx).
        nx = (
            n1 + n2
        )  # This is the nx used in 2-stage IPSID algorithm (without considering X3) i.e., dim([X1;X2])
    ...
    if n2 > 0:
        if nx > iY * ny:
            raise (
                Exception(
                    "nx (currently {}) must be at most iY*ny={}*{}={}. Use a larger horizon iY.".format(
                        nx, iY, ny, iY * ny
                    )
                )
            )
        if (
            "YHatObUfRes_U" not in WS
            or WS["YHatObUfRes_U"] is None
            or "n1" not in WS
            or WS["n1"] != n1
        ):
            WS["n1"] = n1
            ...
            if (
                ...
                YHatOb1, Oy1 = projOblique(Yf, Xk, np.concatenate((WS["Up"], WS["Uf"])))
                Yf = Yf - YHatOb1  # Eq.(25)
            ...
            YHatOb = projOblique(Yf, np.concatenate((WS["Up"], WS["Yp"])), WS["Uf"])[0]
            WS["YHatObUfRes"] = removeProjOrth(YHatOb, WS["Uf"])

            # Orthogonal projection of Yf onto UfUpYp
            WS["YHat"] = projOrth(Yf, np.concatenate((WS["Up"], WS["Yp"], WS["Uf"])))[0]

            # Orthogonal projection of Yf_Minus onto Up_plus,Yp_plus,Uf_Minus
            WS["YHatMinus"] = projOrth(
                Yf_Minus,
                np.concatenate((WS["Up"], WS["Uii"], WS["Yp"], WS["Yii"], Uf_Minus)),
            )[0]

            # Take SVD of YHatObUfRes
            WS["YHatObUfRes_U"], WS["YHatObUfRes_S"], YHat_V = linalg.svd(
                WS["YHatObUfRes"], full_matrices=False, lapack_driver="gesvd"
            )  # Eq.(27)
    ...
    if (
        ...
        YHat = np.concatenate(
            (RYf[:, : ((2 * nu + ny) * iY)], np.zeros((ny * iY, ny))), axis=1
        )
    ...
    if nz > 0:
        if not remove_nonYrelated_fromX1:
            ...
            xHat = s.predict(
                Y if time_first else transposeIf(Y),
                U if time_first else transposeIf(U),
                steady_state=True,
            )[2]
            xHatCat = catIf(transposeIf(xHat), axis=1)
            ZCat = catIf(transposeIf(Z) if time_first else Z, axis=1)
            s.Cz = projOrth(ZCat, xHatCat)[
                1
            ]  # Eq.(40) Fitting Z-readout from all states in case of using additional steps (preprocessing)
            s.Dz = np.zeros(
                (nz, nu)
            )  # Enforcing no feedthrough to z in case of using additional steps (preprocessing)

    # Additional step 2/Learning x3 and its model parameters (if desired): (Vahidi, Sani et al) Fig. S5 bottom row, Note S2
    ####################################################
    if (
        n3 > 0
    ):  # Learn n3 additional stated that optimize the prediction of residual of Z only using past U and past residual of Z
        ...
        params3 = {
            ...
            "Cz": s3.C,
            "Dz": s3.D,
            "Q": np.zeros_like(s3.A),
            "R": np.zeros_like(s.R),
            "S": np.zeros((s3.A.shape[0], s.C.shape[0])),
            "Sxz": s3.S,
            "Syz": np.zeros((ny, nz)),
            "Rz": s3.R,
        }

...
def projOrth(A, B):
    ...
    if B is not None:
        BCov = (
            B @ B.T / B.shape[0]
        )  # Division by num samples eventually cancels out but it makes the computations (specially pinv) numerically more stable
        ABCrossCov = A @ B.T / B.shape[0]
        isOk, attempts = False, 0
        while not isOk and attempts < 10:
            try:
                attempts += 1
                W = ABCrossCov @ np.linalg.pinv(
                    BCov
                )  # or: A / B = A * B.' * pinv(B * B.')
                isOk = True
        ...
        for yi, thisY in enumerate(Y):
            ...
            if yi == 0:
                ...
                if nyThis != ny:
                    raise (
                        Exception(
                            "Size of dimension 1 must be the same in all elements of the data list."
                        )
                    )

...
def render_cross_trial_performance_tab(split_res: Dict[str, Any], sampling_freq: float = 80.0):
...
def render_raincloud_plot(
    ...
    try:
        ...
        df_plot["Block"] = pd.Categorical(df_plot["Block"], categories=unique_blocks, ordered=True)
        ...
        group_colors = {grp: colors[i % len(colors)] for i, grp in enumerate(unique_groups)}
...
def render_normalized_kde_plot(
    ...
    blocks_arr = np.array(blocks) if (blocks and len(blocks) == n_trials) else np.zeros(n_trials)
...
def render_distribution_comparison_plot(
    ...
    for i, grp in enumerate(unique_groups):
        ...
        for blk in unique_blks:
            ...
            blk_true_flat = np.vstack([np.atleast_2d(s) if s.ndim == 1 else s for s in blk_true_samples])
            blk_pred_flat = np.vstack([np.atleast_2d(s) if s.ndim == 1 else s for s in blk_pred_samples])
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.caption("Standardized value distributions compared per group (pooled across blocks).")
...
def render_spectral_fvu_plot(
    ...
    for i, grp in enumerate(unique_groups):
        ...
        fig.add_trace(go.Scatter(
            ...
            y=np.concatenate([mean_ratio + sem_ratio, (mean_ratio - sem_ratio)[::-1]]),
            fill='toself',
        ...
        fig.add_shape(
            ...
            x0=0, x1=max(f), y0=1, y1=1,
            row=row, col=col, xref=f"x{i+1}" if i > 0 else "x", yref=f"y{i+1}" if i > 0 else "y"
        )
...
def render_residual_acf_heatmap(data_true: list, data_pred: list, channel_names: list, title: str, use_bands: bool = False):
    if not data_true or not data_pred or not channel_names: return
    try:
        ...
        if use_bands:
            ...
            for name in channel_names:
                parts = str(name).split("_")
                y_labels.append(parts[2].capitalize() if len(parts) >=3 else parts[1].capitalize() if len(parts)>=2 else "Unknown")
        ...
        fig.update_layout(title=title, xaxis_title="Lag", yaxis_title="Channels", height=500, template="plotly_white")
        st.plotly_chart(fig, use_container_width=True)
        st.caption("Bright colors at lags > 0 indicate temporal structures the model failed to capture.")
    except Exception as e: st.error(f"ACF Error: {e}")

def render_error_cdf_plot(data_true: list, data_pred: list, channel_names: list, title: str, use_bands: bool = False):
    if not data_true or not data_pred or not channel_names: return
    try:
        y_true = np.concatenate([np.atleast_2d(t) for t in data_true], axis=0)
        y_pred = np.concatenate([np.atleast_2d(t) for t in data_pred], axis=0)
        ...
        if use_bands:
            for name in channel_names:
                parts = str(name).split("_")
                groups.append(parts[2].capitalize() if len(parts) >=3 else parts[1].capitalize() if len(parts)>=2 else "Unknown")
        else:
            groups = channel_names
        ...
        fig = px.ecdf(df, x="Abs Error", color="Group", title=title, template="plotly_white")
        fig.update_layout(xaxis_title="Absolute Error |y_true - y_pred|", yaxis_title="Cumulative Probability")
        st.plotly_chart(fig, use_container_width=True)
    except Exception as e: st.error(f"CDF Error: {e}")

def render_residual_qq_plot(data_true: list, data_pred: list, channel_names: list, title: str, use_bands: bool = False):
    if not data_true or not data_pred or not channel_names: return
    try:
        y_true = np.concatenate([np.atleast_2d(t) for t in data_true], axis=0)
        y_pred = np.concatenate([np.atleast_2d(t) for t in data_pred], axis=0)
        ...
        if use_bands:
            for name in channel_names:
                parts = str(name).split("_")
                groups_map.append(parts[2].capitalize() if len(parts) >=3 else parts[1].capitalize() if len(parts)>=2 else "Unknown")
        else:
            groups_map = channel_names
        ...
        for i, grp in enumerate(unique_groups):
            ...
            # Standardize for QQ
            grp_resids = (grp_resids - np.mean(grp_resids)) / (np.std(grp_resids) + 1e-8)
            ...
            fig.add_trace(go.Scatter(x=osm, y=osr, mode='markers', marker=dict(size=3, color='black', opacity=0.5), showlegend=False), row=row, col=col)
            fig.add_trace(go.Scatter(x=[-3, 3], y=[-3, 3], mode='lines', line=dict(color='red', dash='dash'), showlegend=False), row=row, col=col)
        ...
        st.plotly_chart(fig, use_container_width=True)
        st.caption("A straight line indicates Gaussian residuals. Deviations at ends reveal model failure on bursts.")
    except Exception as e: st.error(f"QQ Error: {e}")

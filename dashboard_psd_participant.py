...
# Usage: RESULTS_PATH=/path/to/local/results streamlit run dashboard_psd_participant.py
...
from dashboard.psd_analysis_tab import (
    render_average_psd_participant_level,
    render_multi_participant_psd_analysis,
)
from utils.logger import setup_logger

logger = setup_logger("dashboard_logs", name=__name__)
logger.info("PSD Participant Level Dashboard script started.")

st.set_page_config(layout="wide")

st.title("PSD Analysis - Participant Level")
...
# Participant Level PSD Analysis - doesn't require block_data, but we can get channels from it if available
...
if block_data is not None and not block_data.is_empty():
    ...
    # For now, use common channel names - render functions will filter based on available data
    lfp_channels = [f"LFP_{i}" for i in range(1, 17)]
    ecog_channels = [f"ECOG_{i}" for i in range(1, 5)]

psd_tab, multi_psd_tab = st.tabs(
    ["Participant-level Average PSD", "Multi-Participant PSD"]
)
...
with multi_psd_tab:
    st.markdown("### Multi-Participant PSD Analysis")

...
def load_all_splits(variant_dir: Path, run_ts: str) -> Dict[str, Optional[Dict[str, Any]]]:
...
def compute_classification_for_config(
    ...
    X_test, y_test, meta_test = (
        prepare_epoched_data(test_list, **prep_kwargs) if test_list else (None, None, None)
    )
    ...
    logger.info(f"  DBS ON: {np.sum(y_trainval == 1)} | DBS OFF: {np.sum(y_trainval == 0)}")
    ...
    for clf_name in CLASSIFIERS:
        ...
        key_base = f"{clf_name}_{feature_source}_{mode}_{epoch_params_str}".replace(" ", "_").replace(".", "p")
        if forecast_horizon_sec:
...
def compute(config):
    ...
    logger.info(f"Val results: {val_dir} ({'exists' if val_dir.exists() else 'not found'})")
    logger.info(f"Test results: {test_dir} ({'exists' if test_dir.exists() else 'not found'})")
    
    if not test_dir.exists():
        logger.warning("Test predictions not found. Generating test predictions...")
        logger.info("This is similar to running: python training/test.py --config <config> --run <run_ts>")
        
        from training.components.tester import Tester
        from utils.config import get_config as load_config
        
        training_config_path = project_root / "training" / "setups" / f"{config.run.variant}.yaml"
        if not training_config_path.exists():
            ...
            logger.info("Please run: python training/test.py --config <training_config> --run <run_ts>")
            sys.exit(1)
    ...
    logger.info(f"Forecast parameters from training config: model.forecast.m and model.forecast.history")
    logger.info("="*80)

        ...
        if forecast_target == "Y":
            try:
                ch_use = resolve_neural_y_channel_idx(
                    res_p, neural_y_feature_name, channel_idx
                )
...
plt.show()
print(
    "Fig 64: raincloud — pooled per-trial Y forecast RMSE; per-model panels, DBS-OFF/ON columns."
)
...
plt.show()
print(
    "Fig 66: block-running — chronological 12-block bins; median curve = drift signature."
)

_fc_train = _forecast_at("train")
_fc_val = _forecast_at("val")
fig = _mpl_cascade(
    _fc_train, _fc_val, _fc_test, metric="rmse", target_label="pooled forecast"
)
fig.savefig(str(OUT / "fig_067_pooled_forecast_cascade.png"))
plt.show()
print(
    "Fig 67: cascade — train→val→test per-split strip + median; arrow = chronological-drift gap."
)
...
# %%
def _collect_per_cell_forecast(triplets, *, target="Z", metric="pearson", split="test"):
...
plt.show()
print(
    "Fig 81 (Y self-forecast, lap, pooled): vertical raincloud per framework. DBS-OFF/ON."
)
...
fig.savefig(
    str(OUT / "fig_082_lap_pool_raincloud_yz_forecast.png"), bbox_inches="tight"
)
plt.show()
print(
    "Fig 82 (Y vs Z forecast, lap, pooled): vertical raincloud, Y left + Z right per framework."
)
...
plt.show()
print(
    "Fig 83 (beh forecast): 4 rows × 4 cells. Y self-forecast + Z (behavior) forecast."
)
...
plt.show()
print(
    "Fig 84 (Y self-forecast, beh, pooled): vertical raincloud per framework. DBS-OFF/ON."
)
...
fig.savefig(
    str(OUT / "fig_085_beh_pool_raincloud_yz_forecast.png"), bbox_inches="tight"
)
plt.show()
print(
    "Fig 85 (Y vs Z forecast, beh, pooled): vertical raincloud, Y left + Z right per framework."
)
...
for (m, d), (T, P) in _horizon_arrays.items():
    ...
    style = dbs_style[d]
    label = f"{m} {d.upper()}"
    ax_rmse.plot(x_ms, rmse, color=color, linestyle=style, linewidth=1.4, label=label)
    ax_rmse.fill_between(
        x_ms, rmse - sem, rmse + sem, color=color, alpha=0.12, linewidth=0
    )
...
fig.savefig(str(OUT / "fig_086_pool_horizon_rmse_pearson.png"), bbox_inches="tight")
plt.show()
print(

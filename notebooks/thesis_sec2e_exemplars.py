...
def _score_all_trials(res, ch_idx, target_true, target_pred, metric):
    ...
    for i in range(len(res.get(target_true, []))):
        tt = res[target_true][i]
        pp = res[target_pred][i]
        if tt is None or pp is None:
            continue
        arr_t = np.asarray(tt, dtype=float)
        arr_p = np.asarray(pp, dtype=float)
...
def _pick_best_feat(
    tri, feat_type: str, metric: str, target: str
) -> tuple[str, int, float]:
    ...
    if feat_type == "behavioral":
        ...
        key_t, key_p = (
            ("Z", "Zp") if target == "recon" else ("Z_future_true", "Z_future_pred")
        )
        ...
        key_t, key_p = (
            ("Y", "Yp") if target == "recon" else ("Y_future_true", "Y_future_pred")
        )
    elif feat_type == "lfp":
        ...
        if not names:
            ...
            n_bands = (
                np.asarray(Z0).shape[1]
                if (Z0 is not None and np.asarray(Z0).ndim == 2)
                else 0
            )
...
def _extract_trace_pair(res, ch_idx, trial_idx, key_t, key_p):
    tt = res[key_t][trial_idx]
    pp = res[key_p][trial_idx]
    arr_t = np.asarray(tt, dtype=float)
    arr_p = np.asarray(pp, dtype=float)
...
def _mpl_add_model_with_ci(
    ...
    if res_model is not None:
        ...
        if q_low is not None:
            ...
            ax.fill_between(t_vec[:m], lo, hi, color=color, alpha=0.15, linewidth=0)
...
def build_exemplar_grid(target: str, metric: str, fig_num: int):
    ...
    col_titles = {
        "behavioral": "Behavioural kinematics",
        "neural": "Neural (ECoG top-5)",
        "lfp": "Laplacian LFP",
    }
    key_map = {
        ("recon", "behavioral"): ("Z", "Zp"),
        ("recon", "neural"): ("Y", "Yp"),
        ("recon", "lfp"): ("Z", "Zp"),
    ...
    for ri, tri in enumerate(_all_session_objs):
        for ci, feat_type in enumerate(feat_types):
            ...
            try:
                feat_name, ch_idx, psid_score = _pick_best_feat(
                    tri, feat_type, metric, target
                )
                if ch_idx < 0:
                ...
                t_vec, y_true, y_psid = _extract_trace_pair(
                    res_p, ch_idx, trial_i, key_t, key_p
                )
                if len(y_true) == 0 or not np.any(np.isfinite(y_true)):
                    raise ValueError("empty trace")

                res_v = load_split_results(
                    results_root, tri_use.varma_variant, tri_use.varma_run_ts, "test"
                )
                res_d = (
                    load_split_results(
                        results_root, tri_use.dpad_variant, tri_use.dpad_run_ts, "test"
                    )
                    if tri_use.dpad_run_ts
                    else None
                )
                ...
                if res_v is not None:
                    ...
                    if (
                        ch_v >= 0
                        and key_t in res_v
                        and i_v < len(res_v[key_t])
                        and res_v[key_t][i_v] is not None
                    ):
                        try:
                            tv, _, y_v = _extract_trace_pair(
                                res_v, ch_v, i_v, key_t, key_p
                            )
                            _mpl_add_model_with_ci(
                                ax,
                                tv,
                            ...
                            print(
                                f"  VARMA overlay failed {tri.label}/{feat_type}: {e.__class__.__name__}: {e}"
                            )
                ...
                if res_d is not None:
                    ...
                    if (
                        ...
                        and i_d >= 0
                        and key_t in res_d
                        and i_d < len(res_d[key_t])
                        and res_d[key_t][i_d] is not None
                    ):
                        try:
                            td, _, y_d = _extract_trace_pair(
                                res_d, ch_d, i_d, key_t, key_p
                            )
                            _mpl_add_model_with_ci(
                                ax,
                                td,
                            ...
                            print(
                                f"  DPAD overlay failed {tri.label}/{feat_type}: {e.__class__.__name__}: {e}"
                            )
                ...
                feature_picks.append(
                    (tri.label, feat_type, feat_name, float(psid_score))
                )
                legend_added = True
            except Exception as e:
                panel_label(
                    ax, panel_letter, f"{tri.label} - (no data: {e.__class__.__name__})"
                )
...
for fig_num, target, metric in _EXEMPLAR_PLAN:
    ...
    plt.show()
    print(
        f"Fig {fig_num}: {target} exemplars by {metric_display_name(metric)} — picks:"
    )
    for lbl, ft, feat, score in picks:
        print(
            f"  {lbl:>8}  {ft:<11}  {feat:<32}  PSID {metric_display_name(metric)}={score:.3f}"
        )

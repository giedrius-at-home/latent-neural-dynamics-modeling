...
from thesis_lib.aggregate_rmse import normalize_stim
...
def find_best_trial_indices_per_condition(
    ...
    *,
    ...
    """
    from thesis_lib.aggregate_rmse import normalize_stim
    from thesis_lib.loaders import trial_rmse_y_for_model, trial_rmse_z_for_model
...
def find_best_channel_and_trial(*args, **kwargs):
    """Stub: never implemented. Returns None so callers fall through to
    `find_best_trial_indices_per_condition` with a hard-coded channel.
    Reinstate once an implementation is needed (see thesis_sec2e_exemplars.py
    AUTO_BEST_CHANNEL branch)."""
    return None

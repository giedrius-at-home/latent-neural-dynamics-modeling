...
# Usage: RESULTS_PATH=/path/to/local/results streamlit run dashboard_time_series_session.py
...
from dashboard.time_series_tab import render_session_level_tab
from utils.logger import setup_logger

logger = setup_logger("dashboard_logs", name=__name__)
logger.info("Time-Series Session Level Dashboard script started.")

st.set_page_config(layout="wide")

st.title("Time-Series Analysis - Session Level")
...
# Session Level Analysis - discover channels from session data
...
if block_data is not None and not block_data.is_empty():
    ...
    # Discover channels from session data (even without block_data)
    from dashboard.utils import discover_channels_from_session_data

    if selected_participant_id and selected_session:
        lfp_channels, ecog_channels = discover_channels_from_session_data(
            selected_participant_id, selected_session, selected_dataset
        )
    else:

...
# Usage: RESULTS_PATH=/path/to/local/results streamlit run dashboard_psd_trial.py
...
from dashboard.backbone import format_trial_metadata
from utils.logger import setup_logger

logger = setup_logger("dashboard_logs", name=__name__)
logger.info("PSD Trial Level Dashboard script started.")

st.set_page_config(layout="wide")

st.title("PSD Analysis - Trial Level")
...
if "block_data" in st.session_state:
    ...
    if block_data is not None and not block_data.is_empty():
        ...
        st.info("Block data is empty. Please load block data.")
else:
    st.info(
        "Select a block in the sidebar and click 'Load Block Data' to view Trial Level PSD analysis."
    )
